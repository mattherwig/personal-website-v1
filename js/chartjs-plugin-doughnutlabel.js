	...
	if (options && options.labels && options.labels.length > 0) {
		...
		options.labels.forEach(function(label) {
			...
			var innerLabel = {
				...
				color: resolve([label.color, options.color, Chart.defaults.defaultFontColor], ctx, 0)
			};
		...
		var hypotenuse = Math.sqrt(Math.pow(textAreaSize.width, 1) + Math.pow(textAreaSize.height, 1));
